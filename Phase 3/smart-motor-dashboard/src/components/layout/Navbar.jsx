...
import { Cog, Bell, User, ChevronDown, Menu, Play, Pause, RotateCcw, Sun, Moon } from 'lucide-react';
...
export default function Navbar() {
  ...
  const { unreadNotifications, setMobileSidebarOpen, isDemoMode, toggleDemoMode, resetDemo } = useMonitoring();
  ...
  useEffect(() => {
    const savedTheme = window.localStorage.getItem('motor-theme');
    if (savedTheme === 'bright' || savedTheme === 'dark') {
      setTheme(savedTheme);
      document.documentElement.setAttribute('data-theme', savedTheme);
      document.documentElement.style.colorScheme = savedTheme === 'bright' ? 'light' : 'dark';
    } else {
      const prefersLight = window.matchMedia('(prefers-color-scheme: light)').matches;
      const initialTheme = prefersLight ? 'bright' : 'dark';
      setTheme(initialTheme);
      document.documentElement.setAttribute('data-theme', initialTheme);
      document.documentElement.style.colorScheme = initialTheme === 'bright' ? 'light' : 'dark';
    }
  }, []);
  ...
  const toggleTheme = () => {
    ...
    document.documentElement.style.colorScheme = nextTheme === 'bright' ? 'light' : 'dark';
    window.localStorage.setItem('motor-theme', nextTheme);
  };
      ...
      <header className="sticky top-0 z-40 h-14 bg-industrial-900 border-b border-industrial-700 flex items-center justify-between px-4 lg:px-6">
        ...
        <div className="flex items-center gap-3 text-xs text-industrial-400">
          <div className="flex items-center gap-2">
            <button
              ...
              title={isDemoMode ? 'Turn OFF Demo mode to pause all functions' : 'Turn ON Demo mode to start all functions'}
            >
              {isDemoMode ? <Pause className="w-3.5 h-3.5 fill-emerald-600" /> : <Play className="w-3.5 h-3.5 fill-slate-600" />}
              <span>Demo Mode: <strong className={isDemoMode ? 'text-emerald-700' : 'text-slate-700'}>{isDemoMode ? 'ON' : 'OFF'}</strong></span>
            </button>
            ...
            <button
              ...
              title="Reset motor condition, sensor values, and alerts back to normal baseline"
            >
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            <span className={isDemoMode ? 'text-status-normal font-medium' : 'text-slate-500 font-medium'}>
              {isDemoMode ? 'System Online' : 'Demo Paused'}
            </span>
          </div>
          <span className="hidden lg:inline">Last Update: {timeStr}</span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={toggleTheme}
            className="flex items-center gap-1.5 rounded-full border px-2.5 py-1.5 text-xs font-medium shadow-xs transition-colors bg-[var(--color-industrial-850)] text-[var(--color-industrial-300)] border-[var(--color-industrial-700)] hover:bg-[var(--color-industrial-800)]"
            aria-label={`Switch to ${theme === 'dark' ? 'bright' : 'dark'} view`}
            title={`Switch to ${theme === 'dark' ? 'bright' : 'dark'} view`}
          >

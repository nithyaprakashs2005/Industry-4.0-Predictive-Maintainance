...
function MotorModel({ rpm, status, componentStatus }) {
  ...
  return (
    <group>
      ...
      {[-0.68, 0.68].map((x, idx) => (
        <group key={idx} position={[x, -0.85, 0]}>
          {/* Main Foot Block */}
...
function CanvasResizeHandler({ viewportRef }) {
  ...
  useEffect(() => {
    const updateSize = () => {
      ...
      gl.setSize(width, height, window.devicePixelRatio || 1);
...
export default function MotorVisualization({ sensorData, scenario, motorId = 'MTR-001', motorName = 'Main Production Motor' }) {
  ...
  // Sync state when native fullscreen changes (e.g. Esc key pressed)
  useEffect(() => {
    const handleFullScreenChange = () => {
      const isCurrentlyFullscreen = Boolean(document.fullscreenElement);
      setIsFullScreen(isCurrentlyFullscreen);

      // Force Three.js canvas to recalculate container bounds on exit/enter
  ...
  return (
    <div
      ref={containerRef}
      className={`card-industrial p-4 flex flex-col min-h-0 ${
        isFullScreen 
          ? 'fixed inset-0 z-50 rounded-none h-screen w-screen bg-slate-950 p-6' 
          : 'h-[420px] w-full relative'
      }`}
    >
      <div className="mb-3 flex items-center justify-between">
        ...
        <button
          ...
          type="button"
          className="p-1.5 rounded-lg bg-slate-900 border border-cyan-500/30 text-cyan-400 hover:text-cyan-200 hover:bg-slate-800 hover:border-cyan-400 shadow-md flex items-center gap-1 text-xs cursor-pointer z-10"
      ...
      <div
        ref={viewportRef}
        className="flex-1 h-full min-h-[300px] w-full relative overflow-hidden rounded-lg bg-gradient-to-b from-[#060c1a] to-[#0a1128] border border-cyan-900/40 shadow-inner"
      >
        <Suspense fallback={
          <div className="absolute inset-0 flex items-center justify-center text-cyan-400 text-xs">
          ...
          <div className="absolute inset-0 w-full h-full">
            <Canvas
              resize={{ debounce: 0 }}
              camera={{ position: [3.4, 2.2, 3.4], fov: 38 }}
              className="absolute inset-0 w-full h-full"
              style={{ width: '100%', height: '100%' }}
            >
              <CanvasResizeHandler viewportRef={viewportRef} />
